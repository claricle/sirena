...
. **Builder** ([`lib/sirena/parser/builders/quadrant.rb`](lib/sirena/parser/builders/quadrant.rb:1))
...
. **Parser** ([`lib/sirena/parser/quadrant.rb`](lib/sirena/parser/quadrant.rb:1))
  * Orchestrates grammar and builder
  * Provides error handling

. **Layout** ([`lib/sirena/layout/quadrant.rb`](lib/sirena/layout/quadrant.rb:1))
...
Grammar (Parslet) → Parse Tree
      ↓
Builder → QuadrantChart Model
      ↓
Layout → Graph Structure with SVG Coordinates
      ↓
...
The layout converts normalized coordinates (0-1) to SVG coordinates:
...
* ✅ Coverage against the oracle-valid mermaid-js corpus cases: `--> against valid cases only: 9/9 = 100.0%` (`bundle exec ruby scripts/corpus_sweep.rb --failing quadrant`, BY VERDICT section). The remaining corpus case has an `unknown` verdict in `spec/mermaid/corpus-verdicts.yml`.
