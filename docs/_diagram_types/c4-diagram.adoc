...
Configure diagram layout using [`UpdateLayoutConfig()`](lib/sirena/parser/grammars/c4.rb:64):
...
3. [`Parser::Builders::C4`](lib/sirena/parser/builders/c4.rb:13) - Parse tree to diagram model builder
4. [`Parser::C4`](lib/sirena/parser/c4.rb:29) - Main parser class
5. [`Layout::C4`](lib/sirena/layout/c4.rb:17) - Diagram to graph layout
6. [`Renderer::C4`](lib/sirena/renderer/c4.rb:16) - SVG renderer
7. Layout - Fallback grid positioning (`lib/sirena/engine.rb:302-307`); `elkrb` is a declared dependency but is not called
...
2. **Graph** - Convert to graph structure with positions
...
* Some test-specific constructs may not parse (sprites, links, tags)
* Layout is a fallback grid, not the hierarchical ELK layout this page previously described
* Coverage against the oracle-valid mermaid-js corpus cases: `--> against valid cases only: 11/11 = 100.0%` (`bundle exec ruby scripts/corpus_sweep.rb --failing c4`, BY VERDICT section). The same run reports 22 `artifact`-verdict cases (14 passing) and 14 `unknown`-verdict cases (3 passing), per `spec/mermaid/corpus-verdicts.yml`.
...
* https://github.com/claricle/sirena/blob/main/README.adoc[Main Documentation]
* link:../flowchart/[Flowchart Diagrams]
* link:../sequence-diagram/[Sequence Diagrams]
