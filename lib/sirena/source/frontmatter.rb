...
module Sirena
  class Source
    ...
    #
    # Both the loader's refusal and JS's stringification matter: the WHOLE
    # document is refused over a repeated key or an unresolvable tag
    # wherever it sits (`x: !!float nope` rejects even a fine title), and
    # `title: 0` draws nothing while `title: [0, false, null]` draws
    # "0,false,".
    #
    # Parse to an AST rather than loading so alias resolution stays under
    # Sirena's control — loading would expand aliases before the depth and
    # work bounds below could reject them.
    class Frontmatter
      ...
      # render for mmdc; NUL, a vertical tab and a form feed reach the
      # loader and are refused there — this covers exactly the render set.
      # Psych disagrees on tab: it cannot indent in YAML, so
      # `parse_stream("\t")` raises where mermaid's loader returns nothing.
      BLANK = /\A[ \t\r\n]*\z/

      # Both walks recurse and must stop before the machine stack does — a
      # SystemStackError is not a StandardError, so it escapes every rescue
      # in the pipeline and takes the host down with it. The bound is set
      # for the SMALLEST stack a caller might run on (a Fiber's VM stack is
      # an eighth of a Thread's), not for mermaid's own ~1,400-level limit.
      # A depth bound alone is not enough — a MAPPING level costs more
      # frames than a sequence one, so `title` also catches the overflow
      # directly; this bound only keeps the ORDINARY refusal precise.
      MAX_NESTING = 256
      ...
      # regex would raise ArgumentError out of a method that documents
      # only MalformedFrontmatter.
      def blank?
        @yaml.valid_encoding? && BLANK.match?(@yaml)
      end
      ...
      def resolved(node)
        node.is_a?(Psych::Nodes::Alias) ? bound(node) : node
      end

      # A key is checked and named while the walk is still inside it, so
      # both the nested-key guard and the key's JS name can follow an
      # anchor out to an alias standing LATER in the document that nothing
      # has bound yet. `&a [ { ? *a : x }, *a ]` does exactly that, and it
      # leaked a KeyError out of both title entry points.
      #
      # The anchor standing right now is the one that alias will bind to,
      # so it answers the question. When no anchor stands at all the alias
      # is undefined, and `check` reaches it moments later and refuses it
      # by name — every alias in the document is walked, so that is not a
      # verdict this can swallow.
      def bound(node)
        @bindings.fetch(node) { @anchors[node.anchor] }
      end
      ...
      #
      # `open` holds the sequences this walk is inside, and a sequence
      # joins it as the walk ENTERS it rather than when an alias reaches
      # it. Registering on the alias instead made the bound depend on how
      # the title got to the cycle: `title: &a [x, *a]` drew "x,x," where
      # `a: &a [x, *a]` then `title: *a` drew "x,". Both name the same
      # array, and node prints "x," for it.
