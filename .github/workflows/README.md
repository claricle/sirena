...
|---|---|---|---|
| Fast | `fast-lane` | `unit` (`bundle exec rake` on Ruby 3.3/3.4/4.0-experimental x ubuntu/macos/windows), `pins`, `lint` (`bundle exec rubocop`), `baseline` | < 10 min |
...
needs only `fast-lane` (baseline, the `unit` matrix, pins, lint), matching the
old gate on the test matrix; `links` and the rest of `full-lane` do not gate it,
