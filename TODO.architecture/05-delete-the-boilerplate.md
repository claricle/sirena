...
3. Replace `theme_color(:x) || '#hex'` with `theme_color(:x)`, and make
   ...
   **FIVE of those keys cannot take a single value, so "one value per key"
   is not a sufficient plan.** Measured 2026-09-10 by pairing every
   `theme_color(:x) || "#hex"` in `lib/` with its literal, accepting BOTH
   quote styles:

       axis_line     ["#9ca3af", "#000000"]
       border_color  ["#999", "#666", "#333", "#00796b"]
       grid_line     ["#cccccc", "#666666", "#e5e7eb"]
       text          ["#1f2937", "#000000"]
       text_color    ["#333", "#666", "#000"]

   Re-derive with:

       ruby -Ilib -rsirena -e 'p = Hash.new { |h,k| h[k] = [] }; Dir.glob("lib/**/*.rb").each { |f| File.read(f).scan(/theme_color\(:([a-z_]+)\)\s*\|\|\s*["\x27]([^"\x27]+)["\x27]/) { |k,v| p[k] << v } }; p.each { |k,v| puts "#{k} #{v.uniq.inspect}" if v.uniq.size > 1 }'

   **The character class accepting both quote styles is load-bearing.** An
   earlier version of this command required double quotes and reported
   four ambiguous keys instead of five, because four of `border_color`'s
   five call sites are single-quoted. A derivation that enumerates one
   syntax is a denylist wearing a derivation's clothes.
   ...
   So these five need SEPARATE ROLES, not one key each — the caller wants
   different things and the key name hides that. Name the roles from what
   each call site draws, and amend the permitted-key list to match. The
   other eleven missing keys take a single value and are unaffected.
