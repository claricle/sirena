...
RSpec.describe Sirena::Renderer::ClassDiagram do
  ...
  describe '#render mixed-marker relationships, from real mermaid source' do
    ...
    it 'renders an aggregation-start/inheritance-end marker pair on a solid line (o--|>)' do
      ...
      diamond = polygons.find { |p| polygon_points(p).length == 4 }
      triangle = polygons.find { |p| polygon_points(p).length == 3 }
      expect(diamond.attributes['fill']).to eq('#ffffff')
      expect(nearer_endpoint(polygon_points(diamond), line)).to eq(:start)
      # mermaid's extension/inheritance marker is hollow, not filled.
      expect(triangle.attributes['fill']).to eq('#ffffff')
      expect(nearer_endpoint(polygon_points(triangle), line)).to eq(:end)
    end

    it 'renders a dependency-start/composition-end marker pair on a solid line (<--*)' do
      doc = rendered_document('A <--* B')
      group = relationship_group(doc, 'A_to_B')
      line = REXML::XPath.first(group, 'line')
      expect(line.attributes['stroke-dasharray']).to be_nil
      ...
      dependency = polygons.find { |p| nearer_endpoint(polygon_points(p), line) == :start }
      composition = polygons.find { |p| nearer_endpoint(polygon_points(p), line) == :end }
      dep_points = polygon_points(dependency)
      comp_points = polygon_points(composition)

      expect(dep_points.length).to eq(4)
      expect(dependency.attributes['fill']).to eq('#000000')
      expect(no_three_collinear?(dep_points)).to be(true)
      # Not the same filled shape as composition's diamond.
      expect(rhombus?(dep_points)).to be(false)
      ...
      a_rect = node_rect(doc, 'A')
      expect(dep_points.none? { |pt| inside_rect?(pt, a_rect) }).to be(true)

      expect(comp_points.length).to eq(4)
      expect(composition.attributes['fill']).to eq('#000000')
      expect(rhombus?(comp_points)).to be(true)
    ...
    it 'renders a filled diamond at both ends (*--*)' do
      ...
      expect(polygons.map { |p| polygon_points(p).length }).to all(eq(4))
      expect(polygons.map { |p| nearer_endpoint(polygon_points(p), line) })
    ...
    it 'renders a hollow diamond on a dashed line, no end marker (o..)' do
      ...
      expect(nearer_endpoint(polygon_points(polygons.first), line)).to eq(:start)
    end
