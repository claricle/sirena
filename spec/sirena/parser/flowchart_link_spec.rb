...
RSpec.describe Sirena::Parser::Flowchart do
  ...
  describe "every link mermaid draws" do
    FlowchartLinkSpecHelpers.links.each do |link, type|
  ...
  describe "a link that opens with a dot" do
    # `A.-B` was pinned as a parse error while the whole-id reading was
    # out of reach, which was neither mmdc's single node nor a link. It
    # now reads the node mmdc draws. Measured on mmdc 11.12.0: one
    # `node default` whose id is `A.-B`, and no `flowchart-link` path at
    # all — so the EMPTY edge list is the half that matters, and a link
    # of any type here would be a diagram mermaid does not draw.
    it "is one whole node when it sits flush against the node before it" do
      parsed = parse("A.-B")

      expect(parsed.nodes.map(&:id)).to eq(["A.-B"])
      expect(parsed.edges).to be_empty
    end
