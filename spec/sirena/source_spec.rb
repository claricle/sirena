...
RSpec.describe Sirena::Source do
  describe ".split" do
    ...
    context "with a directive value that is not a bare word" do
      it "scans forward for a terminator and takes the rest of the file" do
        # The comment on DIRECTIVE said only the `{...}` shape scanned
      ...
      it "still stops at the end of a value that starts with a word" do
        expect(described_class.split("%%{init: 1.5\nflowchart LR\n")[:body])
          .to eq(".5\nflowchart LR\n")
      end
    end
  ...
  describe ".title" do
    ...
    describe "a block holding nothing but whitespace" do
      # mmdc renders space, tab, tab+space, newline and CR, and refuses
      # NUL, vertical tab and form feed. Both entry points must agree on
      # that split — never add a `strip.empty?` shortcut to `.title` here,
      # since `String#strip` takes those three characters off and would
      # make it draw no title instead of refusing the document.
      ["\0", " \0\n", "\v", "\f"].each do |yaml|
        it "refuses #{yaml.inspect} through both entry points, as mmdc does" do
          expect { described_class.title(yaml) }
            .to raise_error(Sirena::Source::MalformedFrontmatter, /Malformed/)
          expect { Sirena::Source::Frontmatter.new(yaml).title }
            .to raise_error(Sirena::Source::MalformedFrontmatter, /Malformed/)
        end
      end

      ["   \n", "\t", "\t \t", "\n", "\r"].each do |yaml|
        it "draws no title for #{yaml.inspect}, as mmdc does" do
          # A tab is the one mermaid's loader and Psych disagree about: it
          # cannot indent in YAML, so `Psych.parse_stream("\t")` raises
          # where mmdc renders the diagram.
          expect(described_class.title(yaml)).to be_nil
          expect(Sirena::Source::Frontmatter.new(yaml).title).to be_nil
        end
      end

      it "refuses bytes that are not valid UTF-8 by the documented class" do
        # `strip` raised Encoding::CompatibilityError straight out of the
        # gem here, which `.title` does not document and `Frontmatter` has
        # never let out.
        yaml = (+"title: \xBC*a").force_encoding("UTF-8")

        expect { described_class.title(yaml) }
          .to raise_error(Sirena::Source::MalformedFrontmatter, /Malformed/)
      end

      it "still draws no title for an absent block" do
        expect(described_class.title(nil)).to be_nil
      end
    end
    ...
    describe "a block nested too deeply to walk" do
      ...
      it "still walks mappings a person would write, on a small stack" do
        expect(in_a_fiber(deep_map(150))).to eq("T")
      end

      it "bounds the nesting by depth, not by what the leaf turns out to be" do
        # The depth check sat BELOW the alias early return, so a walk that
        # bottomed out in an alias never reached it. Depth 257 was refused
        # with a scalar leaf and drawn with an alias leaf: the same shape,
        # two verdicts, and the deeper one is the one an attacker picks.
        leaf_alias = "a: &a x\ntitle: #{'[' * 256}*a#{']' * 256}"
        leaf_scalar = "title: #{'[' * 256}x#{']' * 256}"

        expect { described_class.title(leaf_alias) }
          .to raise_error(Sirena::Source::MalformedFrontmatter, /deeply/)
        expect { described_class.title(leaf_scalar) }
          .to raise_error(Sirena::Source::MalformedFrontmatter, /deeply/)
      end
    end
    ...
    describe "anchors and aliases" do
      ...
      it "prints a self-referencing title the once, however it is reached" do
        # A sequence joined the cycle set only when an ALIAS reached it, so
        # a title anchored in place expanded one level further than the same
        # array named through an alias. `node -e 'a=["x"];a.push(a);
        # console.log(String(a))'` prints "x," for both.
        expect(described_class.title("title: &a [x, *a]")).to eq("x,")
        expect(described_class.title("a: &a [x, *a]\ntitle: *a")).to eq("x,")
      end

      it "prints a self-reference in first position as the empty element" do
        # node prints ",x" for `a=[];a.push(a);a.push("x")`; we drew ",x,x".
        expect(described_class.title("title: &a [*a, x]")).to eq(",x")
      end

      it "prints a cycle that closes through a nested sequence the once" do
        # node prints "x," for `a=["x"];a.push([a])`; we drew "x,x,".
        expect(described_class.title("title: &a [x, [*a]]")).to eq("x,")
      end

      it "still repeats an array named twice without a cycle" do
        # The cycle set must not swallow an honest repeat: node prints
        # "x,x" for `e=["x"]; String([e,e])`.
        expect(described_class.title("a: &a [x]\ntitle: [*a, *a]")).to eq("x,x")
      end
      ...
      it "refuses a key that reaches an alias the walk has not met yet" do
        # Checking the key follows `*a` back out to the sequence it names,
        # whose LAST element is an alias nothing has bound yet. Both the
        # guard and the key's own name walked into it and threw a raw
        # KeyError out of the gem instead of refusing the document.
        expect { described_class.title("&a [ { ? *a : x }, *a ]") }
          .to raise_error(Sirena::Source::MalformedFrontmatter, /key/)
      end

      it "names the undefined anchor a key reaches ahead of the walk" do
        # Same shape with a trailing alias that no anchor answers. Naming
        # the key walks into it before `check` gets there, and the verdict
        # has to be the undefined anchor rather than a KeyError.
        expect { described_class.title("&a [ { ? *a : x }, *b ]") }
          .to raise_error(Sirena::Source::MalformedFrontmatter,
                          /undefined anchor: b/)
      end
